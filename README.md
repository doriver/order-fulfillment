...
프로세스
  <div>
    <ol>
      <li> oms에서 특정 주문의 데이터를 wms, tms에게 동시에 전달
      </li>
      <li> wms, tms에서 출고,배송을 위헤 판단후 oms에게 응답
        <ul>
          <li>wms에선 센터별로 물건 준비가 가능한지 판단한후, 센터별 준비 가능시간을 oms에게 응답한다.</li>
          <li>tms에선 가능한 배송 코스들을 판단한후, 배송 코스별 회차들과 상차가능한 센터들을 oms에게 응답한다.  </li>
        </ul>
      </li>
      <li> oms에서 wms, tms에게 받은 응답값을 바탕으로 출고, 배송 결정 
      </li>
